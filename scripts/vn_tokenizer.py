    ...
    if algo == 'mm':
        ...
        for line in sents_:
            ...
            for syl in line:  # Consider each syllable in this line.
                ...
                # Reason: exploit the observation that unknown foreign words are usually clumped together as
                # single words. This improves P by 0.6 %, does not alter R, and improves F-ratio by 0.3 %.
                if 1:
                    all_unk = 1
                    for syl_ in word:
                        if syl_ in words_:
...
if __name__ == '__main__':
    ...
    if n_args < 3:
        _logger.info('\n')
        _logger.info("Vietnamese tokenizer. Version 1.1.")
        _logger.info("==================================")
        _logger.info("Usage: ./vn_tokenizer.py <input file> <output file> <algorithm> <command to run crfsuite binary> <path to libcrfsuite-0.12.so>")
        _logger.info("\t* <input file> and <output file> are mandatory inputs. Rest are optional.")
        _logger.info("\t* Choices for <algorithm> are 'mm' (Maximum Matching) and 'crf' (CRF).")
        _logger.info("\t* Default <algorithm> : CRF.")
        _logger.info("\t* Default command to run crfsuite binary : 'crfsuite'")
        _logger.info("\t* Default path to libcrfsuite-0.12.so : '/usr/local/lib/'")
        _logger.info("\t* The input file must be in UTF-8 encoding.")
        _logger.info("\t* The tokenized text will be written in the output file.")
        _logger.info("\t* Each token will be surrounded by square brackets [].")
        _logger.info("\t* Newlines are assumed to end sentences, so no tokenized word will continue across a newline.")
        _logger.info('\n')
        exit(1)
